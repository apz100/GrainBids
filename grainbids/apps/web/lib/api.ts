...
export const LOCAL_HEADER_AUTH_ENABLED =
  process.env.NEXT_PUBLIC_ENABLE_LOCAL_HEADER_AUTH === "true" && process.env.NODE_ENV !== "production";
...
export function buildApiHeaders(): HeadersInit {
  ...
  const authUserId = (process.env.NEXT_PUBLIC_AUTH_USER_ID || "").trim();
  if (ORG_ID) headers["X-Org-Id"] = ORG_ID;
  if (authUserId) headers["X-Auth-User-Id"] = authUserId;
