...
# ---------- NEW TEST OUTPUT DIR ----------
CANDIDATES = [
    Path(r"P:\Adam\Code\CashGrainBids\testingBids"),
    Path(r"\\DERKS-SERVER\Current\Adam\Code\CashGrainBids\testingBids"),
    Path(__file__).resolve().parent / "testingBids",
]
...
if OUTPUT_DIR is None:
    raise RuntimeError(f"Could not create testingBids directory from {CANDIDATES}: {last_err}")
...
# ---------- ANDERSONS DTN SCRAPER ----------

ANDERSONS_BASE = "https://dtn-api.andersonscanada.com/cash-bids"

# Slug -> pretty location name (extend as needed)
ANDERSONS_LOCATIONS = {
    "blacks-lane": "The Andersons - Blacks Lane",
    "blenheim":    "The Andersons - Blenheim",
    "granton":     "The Andersons - Granton",
    "hensall":     "The Andersons - Hensall",
    "kent-bridge": "The Andersons - Kent Bridge",
    "mitchell":    "The Andersons - Mitchell",
    "norwich":     "The Andersons - Norwich",
    "pain-court":  "The Andersons - Pain Court",
    "pontypool":   "The Andersons - Pontypool",
    "port-albert": "The Andersons - Port Albert",
    "claybanks":   "The Andersons - Claybanks",
    "igpc-ethanol":"The Andersons - IGPC Ethanol",
    "kintore":     "The Andersons - Kintore",
    "rannoch":     "The Andersons - Rannoch",
}


def parse_andersons_html(html: str, location_name: str) -> pd.DataFrame:
    """
    Parse one DTN 'cash-bids/<slug>' HTML page into rows that match
    your OCR schema as closely as possible.

    Output columns:
      Location, Name, Delivery, Delivery End, Futures Month,
      Futures Price, Change, Basis, Bushel Cash Price, MT Cash Price
    """
    dfs = pd.read_html(html)
    if not dfs:
        return pd.DataFrame()

    # Commodity order on these pages is stable
    COMMODITIES = ["Corn", "Soybeans", "White Wheat", "Soft Red Wheat"]
    out_rows = []

    commodity_idx = 0
    for df in dfs:
        # Only tables with metrics like 'Futures Month', 'Basis', etc.
        if df.shape[1] < 2:
            continue
        ...
        def col_like(patterns):
            low_cols = {c.lower(): c for c in wide.columns}
            for p in patterns:
        ...
        col_fut_month   = col_like(["futures month"])
        col_fut_price   = col_like(["futures price"])
        col_fut_change  = col_like(["futures change", "change"])
        col_basis       = col_like(["basis"])
        col_cash        = col_like(["andersons cash price", "cash price"])

        for _, row in wide.iterrows():
            delivery = str(row.get("Delivery", "")).strip()

            rec = {
                "Location":          location_name,
                "Name":              commodity_name,
                "Delivery":          delivery,   # e.g. "2025 Crop"
                "Delivery End":      "",
...
def fetch_andersons_all() -> pd.DataFrame:
    ...
    Returns a single DataFrame matching (most of) your OCR columns.
    """
    all_rows = []
...
def main():
    ...
    out_path = OUTPUT_DIR / f"Testing_CashBids_{fetch_date}.csv"
    ...
    # ---------- ANDERSONS MERGE ----------
    andersons_df = fetch_andersons_all()
    if not andersons_df.empty:
        # Andersons tables don't have Basis Change yet
        if "Basis Change" not in andersons_df.columns:
            andersons_df["Basis Change"] = ""

        # Reorder to match OCR columns
        andersons_df = andersons_df[
            [c for c in base_cols if c in andersons_df.columns]
        ]

        # Tag sources
        out["Source"] = "OCR"
        andersons_df["Source"] = "The Andersons DTN"

        cols_with_source = base_cols + ["Source"]

        # Combine
        out = pd.concat([out, andersons_df], ignore_index=True, sort=False)
        out = out[[c for c in cols_with_source if c in out.columns]]
    else:
        out["Source"] = "OCR"
        cols_with_source = base_cols + ["Source"]
        out = out[[c for c in cols_with_source if c in out.columns]]
